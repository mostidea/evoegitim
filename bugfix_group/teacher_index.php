...
$stmt = $db->prepare("SELECT vbs FROM photos WHERE id = 1");
$stmt->execute();
$photoData = $stmt->fetch(PDO::FETCH_ASSOC);
$coverPhoto = $photoData['vbs'] ? '/uploads/covers/' . $photoData['vbs'] : '/assets/img/default-vbs.jpg';
?>
      <img src="../<?php echo $coverPhoto; ?>" alt="" />
    </div>
    <div class="auth-right py-40 px-24">
      <div class="auth-right__inner mx-auto w-100">
        <a href="https://evoegitim.com/" class="auth-right__logo">
          <img src="assets/images/logo/logo.png" alt="" />
        </a>
        <h2 class="mb-8">Hoş Geldiniz! &#128075;</h2>
        <p class="text-gray-600 text-15 mb-32">Değerli Öğretmenimiz sağlık ve huzur dolu bir gün dileriz.</p>
        <?php if (isset($_GET["error"])) { ?>
          <p class="alert alert-danger">Değerli Öğretmenimiz, email ve şifreniz ile giriş yapamadık. Lütfen bilgilerinizi kontrol ediniz.</p>
        <?php } ?>

        <?php if (isset($_GET["action"]) && $_GET["action"] == 2) { ?>
          <p class="alert alert-success">Değerli Öğretmenimiz, hesap dondurma işleminiz başarıyla tamamlandı.</p>
        <?php } ?>

        <?php if (isset($_GET["action"]) && $_GET["action"] == 1) { ?>
          <p class="alert alert-danger">Değerli Öğretmenimiz, hesap iptal işleminiz başarıyla tamamlandı.</p>
        <?php } ?>

        <?php if (isset($_GET["type"]) && $_GET["type"] == 2) { ?>
          <p class="alert alert-warning">Değerli Öğretmenimiz, hesabınız sizin tarafınızdan donduruldu. Hesabınızın tekrar aktifleştirilmesi için lütfen müşteri hizmetlerimizle iletişime geçin.</p>
        <?php } ?>

        <?php if (isset($_GET["type"]) && $_GET["type"] == 1) { ?>
          <p class="alert alert-warning">Değerli Öğretmenimiz, hesabınız sizin tarafınızdan iptal edildi. Hesabınızın tekrar aktifleştirilmesi için lütfen müşteri hizmetlerimizle iletişime geçin.</p>
        <?php } ?>
        <?php if (isset($_GET["recover"])) { ?>
          <p class="alert alert-success">Değerli Öğretmenimiz, şifre yenileme talebiniz alındı. Lütfen telefon numaranıza gelen yeni şifreniz ile giriş yapınız</p>
        <?php } ?>
        <form action="teacher/auth/login.php" method="POST">
          <div class="mb-24">
          ...
          <div class="mb-32 flex-between flex-wrap gap-8">
            ...
            <a href="teacher/forgot-password.php"
