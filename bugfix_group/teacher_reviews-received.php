...
$reviewsData = $db->prepare("
    SELECT reviews_sended.*, users.fullname, lessons.title AS lit, lessons.level AS leslevel, users.level AS ulevel
    FROM reviews_sended
    INNER JOIN users    ON reviews_sended.student_id = users.id
    INNER JOIN lessons  ON reviews_sended.lesson_id  = lessons.id
    WHERE reviews_sended.teacher_id = ?
    ORDER BY reviews_sended.created_at DESC
");
...
$avatarColors = ['#7c3aed','#0891b2','#059669','#d97706','#db2777','#2563eb'];
?>
<!DOCTYPE html>
<html lang="tr">
<head>
  <?php include "../includes_panel/meta.php"; ?>
  <title>Gelen Değerlendirmeler | Evo Eğitim</title>
  <style>
    :root {
    ...
    .evo-page-icon { width: 48px; height: 48px; border-radius: 14px; background: var(--ev-purple-s); display: flex; align-items: center; justify-content: center; font-size: 1.4rem; color: var(--ev-purple); flex-shrink: 0; }
    ...
    .evo-rc-lesson-name  { font-size: .88rem; font-weight: 700; color: var(--ev-purple); }
    ...
    .evo-rc-note i { font-size: 1rem; color: var(--ev-purple); flex-shrink: 0; margin-top: .05rem; }
  ...
  <div class="dashboard-main-wrapper">
    ...
    <div class="dashboard-body evo-page">
      ...
      <div class="evo-page-header">
        <div class="evo-page-icon"><i class="ph-fill ph-star"></i></div>
        <div>
          <h1 class="evo-page-title">Gelen Değerlendirmeler</h1>
          <p class="evo-page-sub">Öğrencilerinizin ders sonu değerlendirmeleri</p>
        </div>
      </div>
      ...
      <div class="evo-stats">
        <div class="evo-stat-card">
          <div class="evo-stat-icon" style="background:#f5f3ff;color:#7c3aed;"><i class="ph-fill ph-chat-circle-dots"></i></div>
          <div>
        ...
        <div class="evo-stat-card">
          <div class="evo-stat-icon" style="background:#ecfdf5;color:#059669;"><i class="ph-fill ph-thumbs-up"></i></div>
          <div>
            <div class="evo-stat-num"><?php echo $positive; ?></div>
            <div class="evo-stat-lbl">Olumlu Değerlendirme</div>
          </div>
        </div>
        <div class="evo-stat-card">
          ...
          <div>
            ...
            <div class="evo-stat-lbl">Gelişime Açık Alan</div>
          </div>
        </div>
      </div>
      ...
      <?php if (empty($rows)): ?>
        <div class="evo-empty">
          ...
          <p>Henüz hiç değerlendirme almadınız.</p>
        </div>
      <?php else: ?>
        ...
        <div class="evo-rc">
          ...
          <div class="evo-rc-ratings">
            <div class="evo-rating-col">
              <div class="evo-rating-q">Yeterli sayıda soru / örnek çözdü mü?</div>
              <?php echo rateBadge($row['lesson_rate']); ?>
            </div>
            <div class="evo-rating-col">
              <div class="evo-rating-q">Konu anlatımı nasıldı?</div>
              <?php echo rateBadge($row['ready_rate']); ?>
            </div>
            <div class="evo-rating-col">
              <div class="evo-rating-q">Konuya hakimiyet ve teknoloji kullanımı?</div>
